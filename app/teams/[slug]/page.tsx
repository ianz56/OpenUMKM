...
async function getMember(slug: string) {
  const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/data/teams.json`);
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <main className="flex-grow mx-auto max-w-[1000px] w-full px-4 py-12 sm:px-6">
        <Link href="/teams" className="inline-flex items-center text-sm font-semibold text-slate-500 hover:text-blue-600 mb-8">
          <i className="ri-arrow-left-line mr-2"></i> Kembali ke Semua Tim
        </Link>

        <div className="grid gap-12 md:grid-cols-[300px_1fr] items-start">
          <div className="rounded-3xl overflow-hidden border border-slate-200 shadow-xl" data-aos="fade-right">
            <img src={member.image} alt={member.name} className="w-full object-cover aspect-[4/5]" />
          </div>
          ...
          <div className="space-y-6" data-aos="fade-up">
            ...
            <div className="prose prose-slate">
              ...
              <p className="text-slate-600 leading-relaxed border-l-4 border-slate-100 pl-4">"{member.bio}"</p>
            </div>
            <div className="space-y-3">
              <h3 className="text-xl font-bold text-slate-900">Keahlian Utama</h3>
              <div className="flex flex-wrap gap-2">
                {member.skills.map((skill: string) => (
                  <span key={skill} className="rounded-lg bg-slate-100 px-3 py-1.5 text-sm font-bold text-slate-700">{skill}</span>
                ))}
              </div>
            </div>
